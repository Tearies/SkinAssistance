...
namespace SkinAssistance.Core.Native
{
    internal static class PointUtil
    {
        ...
        [SecurityCritical, SecuritySafeCritical]
        public static Point TryClientToRoot(Point point, PresentationSource presentationSource, bool throwOnError,
            out bool success)
        {
            ...
            if (throwOnError || presentationSource != null && presentationSource.CompositionTarget != null &&
                !presentationSource.CompositionTarget.GetPropertyValue<bool>("IsDisposed"))
            {
        ...
        ///     visual into the the coordinate space "below" the visual.
        /// </summary>
        public static Point TryApplyVisualTransform(Point point, Visual v, bool inverse, bool throwOnError,
            out bool success)
        {
            success = true;
        ...
        public static Point ClientToScreen(Point pointClient, PresentationSource presentationSource)
        {
            ...
            var inputSource = presentationSource as HwndSource;
            if (inputSource == null)
                return pointClient;
            var handleRef = new HandleRef(inputSource, inputSource.GetPropertyValue<IntPtr>("CriticalHandle"));

            var ptClient = FromPoint(pointClient);
            var ptClientRTLAdjusted = AdjustForRightToLeft(ptClient, handleRef);
        ...
        internal static Point ScreenToClient(Point pointScreen, PresentationSource presentationSource)
        {
            ...
            var ptClient = FromPoint(pointScreen);
        ...
        internal static Rect ElementToRoot(Rect rectElement, Visual element, PresentationSource presentationSource)
        {
            ...
            var rectRoot = transformElementToRoot.TransformBounds(rectElement);
        ...
        internal static Rect RootToClient(Rect rectRoot, PresentationSource presentationSource)
        {
            ...
            var rectClient = Rect.Transform(rectRootUntransformed, matrixDPI);
        ...
        {
            var corner1 = ClientToScreen(rectClient.TopLeft, hwndSource);
            var corner2 = ClientToScreen(rectClient.BottomRight, hwndSource);
        ...
        {
            var windowStyle = SafeNativeMethods.GetWindowStyle(handleRef, true);
        ...
        internal static RECT AdjustForRightToLeft(RECT rc, HandleRef handleRef)
        {
            ...
            if ((windowStyle & NativeMethods.WS_EX_LAYOUTRTL) == NativeMethods.WS_EX_LAYOUTRTL)
            {
                ...
                var width = rc.right - rc.left; // preserve width
                rc.right = rcClient.right -
                           rc.left; // set right of rect to be as far from right of window as left of rect was from left of window
